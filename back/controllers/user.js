...
//Ajoute une modifie l'image de profil d'origine
    ...
    .then((result) => {
      if (result[0][0].pictureName != null) {
        ...
        fs.unlink(`images/${filename}`, () => {
          database
            .query(
              `UPDATE crud.users SET pictureName='${avatar}' WHERE idusers=${userID}`
            )
...
//Récupère les informations de l'utilisateur à la connection
  ...
  if (req.body.password.length > 6) {
    bcrypt
      ...
      .then((hash) => {
        ...
        database
          .query(
            `UPDATE crud.users SET firstname="${req.body.firstname}",
...
//Récupère les informations utilisateur pour le profil
exports.getUserInfo = (req, res, next) => {
  let userID = req.params["id"];
  database
    .query(
      `SELECT firstname, lastname, pictureName FROM crud.users WHERE idusers=${userID}`
    )
